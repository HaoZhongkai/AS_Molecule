...
def train(args,settings,train_datset, test_dataset, model,optimizer, writer,device):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % settings['cls_epochs'] == 1:
            ...
            time0 = time.time()
            Q = ot.sinkhorn(inst_distr, cls_distr, C, 0.04)  # shape dataset_num*cls_num ...takes 40s~250s on cpu
        ...
        for idx, (mols, n_label, ids)  in enumerate(train_loader):
            ...
            edges_ids = torch.randint(0,g.number_of_edges(),[int(settings['edge_sampling_rate']*np.sqrt(g.number_of_edges()))])
            edge_dist = torch.clone(g.edata['distance'][edges_ids]).requires_grad_(False)
            edge_labels = torch.argmin(torch.abs(edge_dist.unsqueeze(1)-edge_bins),dim=1).long()
            ...
            src_ids, dst_ids = [src_ids[i] for i in edges_ids], [dst_ids[i] for i in edges_ids]
            node_logits = node_classifier(atom_preds[mask])
            edge_logits = edge_classifier(torch.abs(atom_preds[src_ids] - atom_preds[dst_ids]))

            n_pred_cls = torch.argmax(node_logits, dim=1)
            e_pred_cls = torch.argmax(edge_logits,dim=1)
            ...
            n_loss = loss_fn(node_logits, n_label[mask])
            e_loss = loss_fn(edge_logits,edge_labels)
            ...
            n_acc_meter.add(n_pred_cls, n_label[mask])
...
if __name__ == "__main__":
    ...
    if args.use_default is False:
        ...
        args.prop_name = 'homo'
        args.mask_n_ratio = 0.5
    print(args)


    settings = {
        'cls_num':2000,
        'cls_epochs':5,     # clustering every 5epochs
        'iters': 10,
        'init_method':'random',
        'prop_bins':25,
    ...
    info = train(args,settings,train_dataset,test_dataset,model,optimizer, writer, device)
