...
def train(args, settings, train_datset, test_dataset, model, optimizer, writer,
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (mols, n_label, ids) in enumerate(train_loader):
            ...
            edges_ids = torch.randint(0, g.number_of_edges(), [
                int(settings['edge_sampling_rate'] *
                    np.sqrt(g.number_of_edges()))
            ])
            edge_dist = torch.clone(
                g.edata['distance'][edges_ids]).requires_grad_(False)
            edge_labels = torch.argmin(
                torch.abs(edge_dist.unsqueeze(1) - edge_bins), dim=1).long()
            ...
            src_ids, dst_ids = [src_ids[i] for i in edges_ids
                                ], [dst_ids[i] for i in edges_ids]
            node_logits = node_classifier(atom_preds[mask])
            edge_logits = edge_classifier(
                torch.abs(atom_preds[src_ids] - atom_preds[dst_ids]))
        ...
        print(
            "Epoch {:2d}, training: loss: {:.7f}, node loss {}  acc: {:.7f} edge loss {} acc {}  self-clustering: loss: {:.7f} props loss {}  mae {}"
            .format(epoch,
                    loss_meter.value()[0],
