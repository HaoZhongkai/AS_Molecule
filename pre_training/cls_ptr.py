...
def train(args, settings, train_datset, model, optimizer, writer, device):
    ...
    for epoch in range(args.epochs):
        ...
        acc = 100 * sum(n_acc_meter.value()[i, i]
                        for i in range(10)) / n_acc_meter.value().sum()
        print(
            "Epoch {:2d}, training: loss: {:.7f}, acc: {:.7f}  self-clustering: loss: {:.7f} acc: {:.7f}"
            .format(epoch,
...
if __name__ == "__main__":
    ...
    info = train(args, settings, train_dataset, model, optimizer, writer,
                 device)
