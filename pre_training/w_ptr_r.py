...
def train(args,settings,train_datset, test_dataset, model,optimizer, writer,device):
    ...
    cls_log_prob = np.ones([N, K]) * np.log(K) / N  # prob_tensor  (cost function)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % settings['cls_epochs'] == 1:
            ...
            time0 = time.time()
            # cls_tags = ot.sinkhorn(inst_distr, cls_distr, cls_log_prob,0.04)  # shape dataset_num*cls_num ...takes 40s~250s on cpu
        ...
        for idx, (mols, n_label, ids)  in enumerate(train_loader):
            ...
            cls_labels = N * torch.tensor(cls_tags[list(ids)], requires_grad=False).to(device).float()
        ...
        print("Epoch {:2d}, training: loss: {:.7f}, acc: {:.7f}  self-clustering: loss: {:.7f} props loss {}  mae {}".format(epoch, n_loss_meter.value()[0], n_acc, c_loss_meter.value()[0],p_loss_meter.value()[0], p_mae_meter.value()[0]))
...
if __name__ == "__main__":
    ...
    settings = {
        'cls_num':2000,
        'cls_epochs':10,     # clustering every 5epochs
        'iters': 10,
        'init_method':'random',
        'prop_bins':25
    }
    ...
    load_path = '/home/jeffzhu/AL/datasets/models/wsch_g_1205_15_58.pkl'
    ...
    # train_set = MoleDataset(mols=[train_dataset.mols[i] for i in data_ids])
