...
class AL_sampler(object):
    ...
    def _random_query(self):
        query_ids = random.sample(range(len(self.data_ids)),
                                  self.batch_data_num)
    ...
    def _k_center_query(self, inputs):
        ...
        for i in range(len(chunk_ids)):
            min_dist[self.data_ids[
                chunk_ids[i]]] = un_ebd_a[chunk_ids[i]] + torch.min(
                    c_ebd_b -
    ...
    def _k_medroids_query(self, inputs):
        ...
        new_batch_ids_ = k_medoids_pp(un_embeddings,
                                      self.batch_data_num,
                                      10,
                                      show_stats=True)
...
class Inferencer(object):
    ...
    def _bayes_inference(self, model, dataset, device):
        time0 = time.time()
        dataloader = DataLoader(dataset=dataset,
                                batch_size=self.args.batchsize * 13,
                                collate_fn=batcher,
                                shuffle=False,
                                num_workers=self.args.workers)
        model.to(device)
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        if self.method == 'fixed_epochs':
            if type(ft_epochs) is not int:
                print(
                    'fixed epochs finetuning requires a parameter ft_epochs: int'
                )
                raise ValueError
            else:
            ...
            if type(ft_epochs) is not list:
                print(
                    'varying epochs finetuning requires a parameter ft_epochs: list'
                )
                raise ValueError
    ...
    def finetune(self, inputs):
        ...
        if self.method in ('fixed_epochs', 'varying_epochs'):
            info = self._ft_with_known_epochs(train_dataset, model, optimizer,
                                              writer, info, device)

        elif self.method in ('by_valid', ):
            if val_dataset is None:
                raise ValueError
            info = self._ft_by_valid_datas(train_dataset, val_dataset, model,
                                           optimizer, writer, info, device)
        else:
            raise ValueError
    ...
    def _ft_with_known_epochs(self, train_dataset, model, optimizer, writer,
                              info, device):
        train_loader = DataLoader(dataset=train_dataset,
                                  ...
                                  shuffle=self.args.shuffle,
                                  num_workers=self.args.workers)
        print('start finetuning with label numbers {} at iteration {}'.format(
            len(train_dataset), self.iter))
        ...
        for epoch in range(ft_epochs):
            ...
            for idx, (mols, label) in enumerate(train_loader):
                ...
                mae_meter.add(mae.detach().item())
                mse_meter.add(loss.detach().item())
            print("Epoch {:2d}/{:2d}, training: loss: {:.7f}, mae: {:.7f}".
                  format(epoch, self.total_epochs,
                         mse_meter.value()[0],
                         mae_meter.value()[0]))
            ...
            if self.args.use_tb:
                writer.add_scalar('train_loss',
                                  mse_meter.value()[0], self.total_epochs)
                writer.add_scalar('train_mae',
                                  mae_meter.value()[0], self.total_epochs)

        return info

    def _ft_by_valid_datas(self, train_dataset, val_dataset, model, optimizer,
                           writer, info, device):
        train_loader = DataLoader(dataset=train_dataset,
                                  batch_size=self.args.batchsize,
                                  collate_fn=batcher,
        ...
        while True:
            ...
            for idx, (mols, label) in enumerate(train_loader):
                ...
                mse_meter.add(loss.detach().item())
            print(
                "Epoch {:2d}/{:2d}, training: loss: {:.7f}, mae: {:.7f}  validing mae {:.7f}"
                .format(epoch, self.total_epochs,
                        mse_meter.value()[0],
                        mae_meter.value()[0], valid_mae))
...
class Weakly_Supervised_Trainer(object):
    ...
    def run(self,
        ...
        if self.method == 'k_means':
            self._run_kmeans(model, dataset, optimizer, device, writer,
                             p_labels, level)
        elif self.method == 'ot':
            self._run_ot(model, dataset, optimizer, device, writer, p_labels,
                         level)
        else:
            raise ValueError
    ...
    def _run_kmeans(self,
        ...
        n_acc_meter = meter.ConfusionMeter(
            100
        )  # clustering num might be too big, do not use confusion matrix
        ...
        for epoch in range(self.args.ft_epochs):
            ...
            if epoch % settings['cls_epochs'] == 0:
                ...
                if self.iters == 0:
                    cls_tags = k_means(feats_all.cpu(),
                                       settings['cls_num'],
                                       settings['iters'],
                                       inits=settings['init_method'],
                                       show_stats=True)
            ...
            for idx, (mols, n_label, ids) in enumerate(train_loader):
                ...
                mask = torch.randint(
                    0, g.number_of_nodes(),
                    [int(self.args.mask_n_ratio * g.number_of_nodes())])
                ...
                p_acc_meter.add(p_pred_cls, p_labels[list(
                    ids)]) if p_labels is not None else p_acc_meter.add(
                        p_pred_cls,
                        torch.zeros_like(p_pred_cls).long())
                ...
                if idx % 50 == 0 and self.args.use_tb:
                    ...
                    writer.add_scalar(
                        'n_train_acc', acc,
                        int((idx + 1 + epoch * len(train_loader)) / 50))
                    print('training loss {} acc {}'.format(
            ...
            p_acc = 100 * sum(p_acc_meter.value()[i, i] for i in range(
                settings['prop_bins'])) / p_acc_meter.value().sum()
            print(
                "Epoch {:2d}, training: loss: {:.7f}, acc: {:.7f}  self-clustering: loss: {:.7f} acc: {:.7f}  props: loss {} acc {} level {}"
                .format(epoch,
                        n_loss_meter.value()[0], n_acc,
                        c_loss_meter.value()[0], 100 * c_acc_meter.value(),
                        p_loss_meter.value()[0], p_acc, level))
            if (epoch + 1) % 100 == 0:
        ...
        for epoch in range(self.args.ft_epochs):
            ...
            for idx, (mols, n_label, ids) in enumerate(train_loader):
                ...
                cls_labels = torch.tensor(
                    np.argmax(Q[list(ids)],
                              axis=1), requires_grad=False).to(device).long()

                atom, atom_preds, edge_preds, (
                    src, dst,
                    edge_ids), cls_preds, embeddings_g, prop_preds = model(g)

                edge_dist = torch.clone(
                    g.edata['distance'][edge_ids]).requires_grad_(False)
                edge_labels = torch.argmin(torch.abs(edge_dist - edge_bins),
                                           dim=1).long()
                ...
                )) if p_labels is not None else p_mae_meter.add(0)

            # n_loss_test, n_acc_test= test(args,test_loader,model,device)
            n_acc = 100 * sum(n_acc_meter.value()[i, i]
                              for i in range(100)) / n_acc_meter.value().sum()
            e_acc = 100 * sum(e_acc_meter.value()[i, i]
                              for i in range(150)) / e_acc_meter.value().sum()
            print(
    ...
    def generate_p_labels(self, model_h, train_dataset, label_ids,
                          un_labeled_ids, prop_name, device):
        time0 = time.time()
        un_dataset = MoleDataset(
            mols=[train_dataset.mols[i] for i in un_labeled_ids],
            prop_name=prop_name)
        dataloader = DataLoader(dataset=un_dataset,
                                batch_size=self.args.batchsize * 5,
                                collate_fn=batcher,
                                shuffle=False,
                                num_workers=self.args.workers)
        model_h.to(device)
...
def check_point_test(settings, train_dataset, test_dataset, teacher_model,
                     max_epochs, device):

    dim, cutoff, output_dim, width, n_conv, norm, atom_ref, pre_train = settings[
        'dim'], settings['cutoff'], settings['output_dim'], settings[
            'width'], settings['n_conv'], settings['norm'], settings[
                'atom_ref'], settings['pre_train']
    lr, epochs, batch_size, n_patience = settings['lr'], settings[
        'epochs'], settings['batch_size'], settings['n_patience']
    model = SchNetModel(dim=dim,
                        cutoff=cutoff,
    ...
    for i in range(max_epochs):
        ...
        if test_mae[-1] > best_test_mae:
            ...
            patience = 0
        print('checkpoint test mae {} patience {}'.format(
            test_mae_meter.value()[0], patience))
